...
```{stan model, output.var="stan_model",message=F, results="hide"}
data {
  int<lower=1> N;
  vector[N] x;
  int<lower=0> y[N,2];
...
```{r fit_model, results=FALSE, echo=TRUE}
model_fit <- sampling(stan_model, data=stan_data)
```

We can get an idea as to whether the samples have converged by examining the ($\hat{r}$)[http://projecteuclid.org/download/pdf_1/euclid.ss/1177011136] diagnostic values.
